...
assert.equal(model.normalizeTcpEndpoint("192.168.1.50").host, "192.168.1.50")
assert.equal(model.normalizeTcpEndpoint("192.168.1.50").port, 5000)
assert.equal(model.normalizeTcpEndpoint("192.168.1.50:4403").host, "192.168.1.50")
assert.equal(model.normalizeTcpEndpoint("192.168.1.50:4403").port, 4403)
assert.equal(model.normalizeTcpEndpoint("tcp://mesh.local:6000").host, "mesh.local")
assert.equal(model.normalizeTcpEndpoint("tcp://mesh.local:6000").port, 6000)
assert.equal(model.normalizeTcpEndpoint("[::1]:5001").host, "::1")
assert.equal(model.normalizeTcpEndpoint("[::1]:5001").port, 5001)
assert.equal(model.normalizeTcpEndpoint("bad host name").host, "127.0.0.1")
assert.equal(model.normalizeTcpEndpoint("bad host name").port, 5000)
assert.equal(model.tcpHost("tcp://10.0.0.2:8000"), "10.0.0.2")
assert.equal(model.tcpPort("", "10.0.0.2:8000"), 8000)
...
assert.equal(model.safeCliError("connection refused", false, "tcp"), "Connection refused by the TCP companion")
assert.equal(model.safeCliError("Connect call failed ['127.0.0.1', 5000]", false, "tcp"), "Connection refused by the TCP companion")
assert.equal(model.safeCliError("Name or service not known", false, "tcp"), "TCP companion hostname could not be resolved")
assert.equal(model.safeCliError("No route to host", false, "tcp"), "TCP companion network unreachable")
assert.equal(model.safeCliError("Connection reset by peer", false, "tcp"), "TCP companion closed connection")
